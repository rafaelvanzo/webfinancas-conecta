...
*/
      ...
      if($params['idConsulta'] != NULL || $params['idConsulta'] != '')
      {
          ...
          $date = new DateTime($dataConsulta['Horario'], new DateTimeZone( 'America/Sao_Paulo'));
          $date->modify("-".$tempo." minutes");
...
public function LiberarHorariosAnterior($horarioIndiposnivel, $tempo)
{
    ...
    {
       $date = new DateTime($horarioIndiposnivel[$key]['horarioInicial'], new DateTimeZone( 'America/Sao_Paulo'));
       $date->modify("-".$tempo." minutes");
...
public function diferencaMinutos($start, $end, $current)
{
    ...
    $date = new DateTime($end, new DateTimeZone( 'America/Sao_Paulo'));
    $endTime = $date->modify("-1 minutes");
...
public function ComunicarFinanceiro($idConsulta)
{
    ...
    {       
        ...
        $favorecido = (!empty($dados['IdResponsavel'])) ? $dados['IdResponsavel'] : $dados['IdFavorecido'];
        $responsavel = (!empty($dados['IdResponsavel'])) ? $dados['IdFavorecido'] : '';

        $dados = array('descricao' => '** '.$descricao,                        
                       'dt_emissao' => $dados['DataInicial'], 
                       'dt_vencimento' => $dados['DataInicial'], 
                       'favorecido_id' => $favorecido, 
                       'favorecido_id_dep' => $responsavel,
                       'parcela_numero' => 1, 
