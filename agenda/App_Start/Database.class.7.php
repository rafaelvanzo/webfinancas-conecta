...
#-#############################################
# desc: constructor
function __construct($server,$user,$pass,$database,$pre = false){
	$this->server	= $server;
	$this->user		= $user;
	$this->pass		= $pass;
...
function fetch_array($query_id=-1) {
	// retrieve row
	/* Removido quando convertido para PHP 7 <-=****
	if ($query_id!=-1) {
...
function oops($msg='') {
	...
	?>
		...
		<?php if(strlen($this->error)>0) echo '<tr><td align="right" valign="top" nowrap>mysqli Error:</td><td>'.$this->error.'</td></tr>'; ?>
...
//total
      function numRows($query){      
        $q        = mysqli_query($this->link_id, $query);
