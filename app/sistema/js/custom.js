	    ...
	    buttons: {
	        Salvar: function () {
	            if ($('#formfavAC').valid()) {
					...
					//Adiciona o required para os recebimentos na janela Adicionar Favorecidos se o carne leão estiver habilitado.
					if(campo_id == 'form_pgto_favorecido_id' && $('#dialog-favorecido-ac .favCPF_CNPJ').hasClass('carneLeao')){
						$('#dialog-favorecido-ac .favCPF_CNPJ').addClass('required');
					}
...
*/	
	...
	$( ".favorecido_buscar" ).autocomplete({
		...
		select: function( event, ui ) {
			...
			/** Inativar a obrigação do preenchimento do CPF/CNPJ, mesmo quando a conta carne leão estiver ativada */
			if(campo_id == 'form_pgto_favorecido_id' && $('#dialog-favorecido-ac .favCPF_CNPJ').hasClass('carneLeao')){
				$('#dialog-favorecido-ac .favCPF_CNPJ').removeClass('required');
			}
	...
	var plc_source_url_rcbt = "php/plano_contas_buscar.php?tp_lanc=1";
	var plc_source_rcbt = function (request, response) {
	    $.getJSON(plc_source_url_rcbt, request, function (data, status, xhr) {
	        response(data);
	    });
	}

	var plc_source_url_pgto = "php/plano_contas_buscar.php?tp_lanc=2";
	var plc_source_pgto = function (request, response) {
	    $.getJSON(plc_source_url_pgto, request, function (data, status, xhr) {
	        response(data);
	    });
	}
...
function favorecidosIncluirAc(nome, campo_id, inscricao, cpf_cnpj) { 
	...
	//Verifica se é um lancamento pagamento e não valida se o cpf foi preenchido, caso queira o favorecido seja validado para o carne leão no pagamento, comente essa linha.
	var tpform = campo_id;

	var params = "funcao=favorecidosIncluirAc";
	params += "&nome=" + nome + "&inscricao=" + inscricao + "&cpf_cnpj=" + cpf_cnpj + "&tpform=" + tpform;
	$.ajax({
		type: 'post',
		...
		success: function(data){
			...
			var dados = JSON.parse(data); 
				...
				if(dados.status == "1"){
					...
					var campo_text = campo_id.split('_id', 1);
					$('#' + campo_text).val(nome);
...
function recibo(form_id,tipo){
	...
	params += '&tipo='+tipo;
	var win = window.open("/sistema/modulos/lancamento/php/funcoes.php?"+params, '_parent');
	win.focus();
}
	...
	$.ajax({
		...
		success: function(data){
			if(ctr_id!=0){
				...
				$('#'+form_id+' span.check-green').eq(4).css('display','block');
			}
			if(plc_id!=0){
...
/** Retornar para o sistema do contador. */
/** LOGIN ENTRE CONTABILIDADE E FINANCEIRO */
function acessarConador(id)
{
	...
	$.ajax({
        type: 'post',
        url: '../contador/modulos/clientes/php/funcoes.php?funcao=login',
        data: { id: id },
        dataType: 'json',
        cache: true,
        success: function (dados) {
            ...
            if(dados.situacao == '1'){
                ...
                alert('Retornando para o sistema do contador.');
                window.location.href = "../contador/clientes";
