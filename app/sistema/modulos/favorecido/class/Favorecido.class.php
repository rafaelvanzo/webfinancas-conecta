...
*/
	...
	function favorecidosIncluir($db){
		...
		$favorecido['dtNascimento'] = self::FormatDateDB($favorecido['dtNascimento']);

		$favorecidoId = $db->query_insert('favorecidos',$favorecido);
		$retorno = array('situacao'=>1,"notificacao"=>"Favorecido cadastrado com sucesso.", "id" => $favorecidoId);
		return $retorno;
	}
...
*/
	...
	function favorecidosIncluirAc($db,$dados){
		...
		$cpf = $db->fetch_assoc('SELECT cpf_cnpj FROM favorecidos WHERE cpf_cnpj = "'.$dados['cpf_cnpj'].'"');

		if($dados['tpform'] == 'form_pgto_favorecido_id' || $dados['cpf_cnpj'] == '' || $cpf == false ){

			$favorecido_id = $db->query_insert('favorecidos',array("nome"=>$dados['nome'], "inscricao"=>$dados['inscricao'], "cpf_cnpj"=>$dados['cpf_cnpj']));			

			return array('status'=>1, "favorecido_id"=>$favorecido_id,"notificacao"=>"Favorecido cadastrado com sucesso.");

		}else{

			return array('status'=>0, "notificacao"=>"Favorecido já possui cadastro");
...
*/
	...
	function favorecidosVisualizar($db,$favorecido_id){
		$favorecido_visualizar = $db->fetch_array($db->query("select *, DATE_FORMAT(dtNascimento, '%d/%m/%Y') as dtNascimento from favorecidos where id = ".$favorecido_id));
...
*/
    ...
    function DataTableAjax($db,$params){
        ...
        $iTotalDisplayRecords = $db->numRows('select id from favorecidos');
