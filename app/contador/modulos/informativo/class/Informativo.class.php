...
function DataTableAjax($db_wf,$params){
    ...
    $dt_final = $dt_final[2].'/'.$dt_final[1].'/'.$dt_final[0];
        ...
        if(strtotime($dados['dt_final']) < strtotime(date('Y-m-d'))){
            $registro_inativar['situacao'] = 0; $teste = strtotime($dados['dt_final']);
            $db_wf->query_update('clientes_informativo', $registro_inativar, ' id = '.$dados['id']);
...
function visualizarInfo($db_wf, $params){
   ...
   $retorno = array("titulo"=>$registro['titulo'], "dt_inicio"=>$registro['dt_inicio'], "dt_final"=>$registro['dt_final'], "situacao"=>$registro['situacao'], "descricao"=>$registro['descricao']);
