...
function DataTableAjax($db, $db_wf, $db_w2b, $params){
    ...
    if($sSearch==""){
        ...
        $query_dataTable = "SELECT * FROM chat_categorias WHERE contador_id = ".$cliente_id;

    }else{
        //Query princial
        $query_dataTable = "SELECT * FROM chat_categorias WHERE contador_id = ".$cliente_id." AND assunto like '%".$sSearch."%'";
    }   
    ...
    foreach($array_dados as $dados){	
       ...
       if
        ...
        $qtd_msg_novas = mysql_num_rows(mysql_query('SELECT id FROM chat_mensagens WHERE chat_categoria_id = '.$dados['id'].' AND visualizada = 0 AND remetente_id != '.$cliente_id, $db_wf->link_id));
        ...
        $registros_tabela = '<span class="tabelaLink" onClick="abrirMensagem('.$dados['id'].');" >
                                    ...
                                    <span class="lDespesa tbWF">
					                    <strong class="blue">'.$nome['nome'].':</strong>  <b>'.$dados['assunto'].' </b> - <b class="green">'.$situacao.'</b>
...
function abrirMensagem( $db_wf, $params){
    ...
    $cliente_id2 = $db_wf->fetch_assoc("SELECT cliente_id, situacao, DATE_FORMAT(dt_conclusao, '%d/%m/%Y %H:%i:%s') as dt_conclusao FROM chat_categorias WHERE id = ".$params['chat_categoria_id']);
   ...
   if($cliente_id2['situacao'] == true){ 
       $chat_categoria_situacao = $cliente_id2['situacao']; 
       $dt_conclusao = $cliente_id2['dt_conclusao'];
   }else{
       $chat_categoria_situacao = $chat_categoria['situacao']; 
       $dt_conclusao = $chat_categoria['dt_conclusao'];
   }
...
function concluirSolicitacao($db_wf, $params){
         
    $registro['situacao'] = 2; // situacao = 2 -> Chat concluido.
    $registro['dt_conclusao'] = date('Y-m-d H:i:s');
    $db_wf->query_update("chat_categorias", $registro,"id =".$params['chat_categoria_id'] );
