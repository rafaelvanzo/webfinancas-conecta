...
require_once('../../../../sistema/modulos/usuario/class/Usuario.class.php');
...
function DataTableAjax($db_w2b,$params){
    ...
    $registros_tabela = '<span>
                                    ...
                                    <div class="tbWFoption" style="z-index:999;">
                                        ...
                                        <a href="javascript://void(0);" original-title="Alterar Senha" class="smallButton btTBwf tipS" onClick="alterarSenha('.$dados['id'].');"><img src="../../sistema/images/icons/dark/locked2.png" width="10"></a>
                                        <a href="javascript://void(0);" original-title="Acessar financeiro do cliente" class="smallButton btTBwf tipS" onClick="acessarFinancas('.$dados['id'].');"><img src="../../sistema/images/icons/dark/key.png" width="10"></a>
				                    </div>   
...
function add($db, $db_wf, $db_w2b, $params){
    ...
    if(!$cliente_id){
        ...
        $array_cadastro['agenda'] = $params['agenda']; 
...
function excluir($db_w2b, $db_wf, $db, $params){   
    ...
    /** Altenar entre o sistema da contabilidade e dos clientes */
    public function loginCliente($dbW2b, $dbWf, $id)
    {
        $cliente_email = $dbW2b->fetch_assoc('SELECT nome, email, cpf_cnpj FROM clientes WHERE id = '.$id['id']);
        $params = $dbWf->fetch_assoc('SELECT email, senha, nome FROM usuarios WHERE cliente_id = '.$id['id'].' AND email = "'.$cliente_email['email'].'"');

        /** Atriui a id da contabilidade para a SESSION retorno_parceiro_id */
           if(isset($_SESSION['retorno_parceiro_id'])){
            unset($_SESSION['retorno_parceiro_id']);
            $_SESSION['email'] = $_SESSION['email_old'];
            unset($_SESSION['email_old']);
           }else{
            $_SESSION['retorno_parceiro_id'] = $_SESSION['cliente_id'];
            $_SESSION['email_old'] = $_SESSION['email'];
            $_SESSION['email'] = $_SESSION['email'].' ( Logado como: '.$params['email'].' )';
           }

          

            $login = new Usuario();
            $login->AlternarLicenca($dbWf, $id['id']);

            $_SESSION['nome'] = $cliente_email['nome'];
            $_SESSION['cpf_cnpj'] = $cliente_email['cpf_cnpj'];
                
        return array('situacao' => 1, 'email' => $params['nome'].' ( '.$params['email'].' )', 'teste' => $_SESSION['retorno_parceiro_id']);
    }
