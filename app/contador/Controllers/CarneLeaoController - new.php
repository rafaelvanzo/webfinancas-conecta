...
class CarneLeaoController
{
    ...
    function CriarArquivoCSV($Cliente_id, $AnoDeclaracao, $AnoCompetencia)
    {
        ...
        $queryLinhas = "SELECT a.favorecido_id_dep, date_format(a.dt_compensacao, '%d/%m/%Y') as data, replace(a.valor, '.', '') as valor, replace(replace(b.cpf_cnpj,'-',''),'.','') as cpf_cnpj, b.inscricao, a.descricao
                        FROM lancamentos a
                        JOIN favorecidos b ON a.favorecido_id = b.id
                        WHERE a.dt_compensacao >= '$AnoCompetencia-01-01' AND a.dt_compensacao <= '$AnoCompetencia-12-31'
        ...
        foreach($Linhas as $Linhas){
            ...
            $Valor = str_pad($Linhas['valor'],13, '',STR_PAD_LEFT);
            
			//if(strlen($Linhas['cpf_cnpj']) == 11)
                $ConteudoArquivo .= $Linhas['data'].';'.$codigoRendimento.';;'$Valor.';;'.$descricao.';'.$inscricao.';'.$Linhas['cpf_cnpj'].$dep_cpf."\r\n";
        ...
        // Criar arquivo CSV
        $nomeDeclarante = $this->db_w2b->fetch_assoc('SELECT nome, replace(replace(cpf_cnpj,"-",""),".","") as cpf_cnpj FROM clientes WHERE id ='.$Cliente_id);

        //Nome do arquivo
        $NomeArquivo = 'exportar_carne_leao_'.$declarante['cpf_cnpj'].'_'.$declarante['nome'].'_'.$AnoDeclaracao.'_'.$AnoCompetencia;
