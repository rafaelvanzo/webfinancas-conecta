...
class CarneLeaoController
{
    ...
    function ArquivoLinhasValores($ContasFinanceirasCarneLeao, $AnoCompetencia, $NomeArquivo){ 
        ...
        $queryLinhas = "SELECT a.favorecido_id_dep, date_format(a.dt_compensacao, '%m%Y') as data, replace(a.valor, '.', '') as valor, replace(replace(b.cpf_cnpj,'-',''),'.','') as cpf_cnpj
                    FROM lancamentos a
                    JOIN favorecidos b ON a.favorecido_id = b.id
                    WHERE a.dt_compensacao >= '$AnoCompetencia-01-01' AND a.dt_compensacao <= '$AnoCompetencia-12-31'
                        ...
                        AND a.tipo = 'R'
                        AND b.inscricao = 'cpf'
                        ORDER BY a.dt_compensacao DESC, a.valor";
        ...
        foreach($Linhas as $Linhas){
            ...
            if($Linhas['favorecido_id_dep'] > 0){

                $dep = $this->dbCliente->fetch_assoc("SELECT replace(replace(replace(cpf_cnpj,'-',''),'.',''), '/','') as cpf_cnpj FROM favorecidos WHERE id =".$Linhas['favorecido_id_dep']); 
               
                $dep_cpf = (strlen($dep['cpf_cnpj']) < 14 ) ? $dep['cpf_cnpj'] : $Linhas['cpf_cnpj'];

                //$dep_cpf = $dep['cpf_cnpj'];

            }else{

                $dep_cpf = $Linhas['cpf_cnpj'];
			...
			if(strlen($Linhas['cpf_cnpj']) == 11)
                $Retorno .= '03'.$Linhas['data'].$NumDesc.$Linhas['cpf_cnpj'].$dep_cpf.$Valor."\r\n";
    ...
    function CriarArquivo($ClienteId, $AnoDec){
        ...
        $VersaoCarneLeao = array('2016' => '110', '2017' => '110', '2018' => '110', '2019' => '100', '2020' => '100');		/* Versão do Carne Leão */
	...
	function ExportarArquivo(){
        ...
        $dadosFavorecidos = $this->dbCliente->fetch_all_array("
                ...
                from favorecidos a
                join lancamentos b on a.id = b.favorecido_id OR a.id = b.favorecido_id_dep
        ...
        if($c > 0){
            ...
            if($AnoDeclaracao >= "2022")
                $NomeArquivo = self::CriarArquivoCSV($Cliente_id, $AnoDeclaracao, $AnoCompetencia);
            
            else //Se estiver tudo certo gera o arquivo.
    ...
    function CriarArquivoCSV($Cliente_id, $AnoDeclaracao, $AnoCompetencia)
    {
        //Seleciona as contas financeiras habilitadas para o carne leão e cria uma variavel separada por virgula
        $contasFinanceirasCarneLeao = $this->dbCliente->fetch_all_array('SELECT id FROM contas WHERE carne_leao = 1');
        $contasFinanceirasId = array();
        foreach($contasFinanceirasCarneLeao as $contaFinanceiraId){
            array_push($contasFinanceirasId,$contaFinanceiraId["id"]);
        ...
        //Busca todos os registros referente a competnecia 
        $queryLinhas = "SELECT a.favorecido_id_dep, date_format(a.dt_compensacao, '%d/%m/%Y') as data, valor, replace(replace(b.cpf_cnpj,'-',''),'.','') as cpf_cnpj, b.inscricao, a.descricao
                        FROM lancamentos a
                        JOIN favorecidos b ON a.favorecido_id = b.id
                        WHERE a.dt_compensacao >= '$AnoCompetencia-01-01' AND a.dt_compensacao <= '$AnoCompetencia-12-31'
                        AND a.conta_id in ($contasFinanceirasCarneLeaoId)
                        AND a.compensado = 1 
                        AND a.tipo = 'R'
                        AND b.inscricao = 'cpf'
                        ORDER BY a.dt_compensacao DESC, a.valor";
        ...
        foreach($Linhas as $Linhas){
            ...
            if($Linhas['favorecido_id_dep'] > 0){
                ...
                $dep = $this->dbCliente->fetch_assoc("SELECT replace(replace(replace(cpf_cnpj,'-',''),'.',''), '/','') as cpf_cnpj, inscricao FROM favorecidos WHERE id =".$Linhas['favorecido_id_dep']); 
               
                $dep_cpf = (strlen($dep['cpf_cnpj']) < 14 ) ? $dep['cpf_cnpj'] : $Linhas['cpf_cnpj'];
                
                $inscricao = $dep['inscricao'];

            }else{

                $dep_cpf = $Linhas['cpf_cnpj'];

                $inscricao = $Linhas['inscricao'];
            ...
            $descricao = self::removeCaracteresEspeciais($Linhas['descricao']);

            $Valor = number_format($Linhas['valor'],2,",","");
            
            //Valida se o CPF tem 11 digitos
            $cpf_count = strlen($Linhas['cpf_cnpj']);
			if($cpf_count == 11 && $Linhas['cpf_cnpj'] !== '00000000000')
                $ConteudoArquivo .= $Linhas['data'].';'.$codigoRendimento.';'.$codigoOcupacao.';'.$Valor.';;'.$descricao.';'.$inscricao.';'.$Linhas['cpf_cnpj'].';'.$dep_cpf."\r\n";
        ...
        // Criar arquivo CSV
        $declarante = $this->db_w2b->fetch_assoc('SELECT nome, replace(replace(cpf_cnpj,"-",""),".","") as cpf_cnpj FROM clientes WHERE id ='.$Cliente_id);

        $nomeDeclarante = self::clearString($declarante['nome']);

        //Nome do arquivo
        $NomeArquivo = 'exportar-carne_leao-'.$declarante['cpf_cnpj'].'-'.$nomeDeclarante.'-'.$AnoDeclaracao.'-'.$AnoCompetencia;
        ...
        //Escreve no arquivo aberto.
        fwrite($fp, $ConteudoArquivo);		//Quebra de linha TXT "\r\n"
        
        
        //Fecha o arquivo.
        fclose($fp);

        //Retornar o arquivo preenchido
        return $arquivo;
    }
